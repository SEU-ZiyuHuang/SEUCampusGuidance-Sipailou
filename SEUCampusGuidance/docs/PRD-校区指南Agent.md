...
1. **六校区结构化 md 文档**（`原校区指南-md文档整理/`，共 7.5 万字）：由 2025 版指南图片逐张人工整理而成，统一包含元数据头、按主题分层的章节、`[源图 Pxx]` 页码溯源、内页信息差异记录，以及专为 Agent 场景撰写的「使用建议」与回答模板。
...
| 校区 | 字符数 | 估算 token（约 0.6–0.7 token/字） |
| --- | ---: | ---: |
| 九龙湖 | 19,449 | ~1.2–1.4 万 |
| 四牌楼 | 13,183 | ~0.8–0.9 万 |
| 无锡 | 12,853 | ~0.8–0.9 万 |
| 丁家桥 | 11,090 | ~0.7–0.8 万 |
| 苏州 | 10,500 | ~0.6–0.7 万 |
| 江北 | 8,026 | ~0.5–0.6 万 |
| **合计** | **75,101** | **~4.5–5.3 万** |

> 勘误：v1.0 此表误用了 `wc -m` 在非 UTF-8 locale 下的输出，那是**字节数**（合计 162,818）而非字符数。真实字符数如上，约为原数据的 46%。结论方向不变，且更成立。

关键推论：

- **单校区全文可直接放入上下文**（最大的九龙湖仅约 1.4 万 token，DeepSeek V4 Flash 的窗口远大于此），意味着即使检索完全失效，「兜底注入整个校区」也是免费的保险。
- **六校区全量仍不宜每次都塞**：约 5 万 token/请求，成本、延迟以及长上下文下的信息淹没都不划算。
- 因此问题的实质不是「要不要向量库」，而是「**如何把范围先缩小到单校区，再缩小到相关章节**」——这用确定性路由 + 关键词检索即可解决，不需要 embedding。

实际切块产出（`agent/scripts/build-knowledge.mjs`）：76 个章节块，可检索正文 56,106 字符，其中 75 块带源图页码，最长块 1,643 字符，生成的知识模块约 170 KB。
...
| API 成本失控 | Key 被刷爆、社团经费受损 | Key 仅服务端；F-08 IP 频控 + 输入长度限制 + 上下文 token 上限；成本按「单次提问 = 1—6 次 API 调用（预检索够用时 1 次，每多一次工具调用多 1 次，另加 1 次收手决策轮）× 输入 4–6k token + 输出 ≤1k token」结合 DeepSeek 官网现行单价估算，设置日用量告警阈值 |
...
| 文件 | 校区 | 版本 | 源图 | 字符数 | 章节块 | 备注 |
| --- | --- | --- | ---: | ---: | ---: | --- |
| `东南大学九龙湖校区指南_2025版整理.md` | 九龙湖 | 2025.09 | 16 张 | 19,449 | 20 | 含三条接驳车完整时刻表、兰台公寓专章 |
| `东南大学四牌楼校区指南_2025版整理.md` | 四牌楼 | 2025.08 | 11 张 | 13,183 | 14 | 含 16 栋文昌宿舍逐栋房型、AED 点位 |
| `东南大学无锡校区指南_2025版整理.md` | 无锡 | 2025（25版） | 8 张 | 12,853 | 13 | 含无人小巴时刻、分校门地铁路线 |
| `东南大学丁家桥校区指南_25版整理.md` | 丁家桥 | 25版（无月份） | 6 张 | 11,090 | 13 | 含中大医院小门、跨校区路线 |
| `东南大学苏州校区指南_2025版整理.md` | 苏州 | 2025（25版） | 7 张 | 10,500 | 10 | 含水电费、慧湖通报修、苏州交通 |
| `东南大学江北校区指南_2025版整理.md` | 江北 | 2025（25版） | 5 张 | 8,026 | 6 | 含西/东区结构、机场巴士 |
| 合计 | 6 校区 | — | 53 张 | 75,101 | 76 | 均含差异项记录与 Agent 使用建议 |

「章节块」为 `agent/scripts/build-knowledge.mjs` 的实际切块数，可通过 `npm run build:knowledge` 重建。
